...
import { reuseSpecialProps, wrapInitializer, parseDependencyDeclaration, name, autoName, type, inject, autoInject, alsoInject, useInject, mergeInject, parseInjections, singleton, extra, initializer, constant, service, autoService, provider, autoProvider, handler, autoHandler, SPECIAL_PROPS } from './util';
...
describe('wrapInitializer', () => {
  it('should work with a service initializer', async () => {
    ...
    const newInitializer = wrapInitializer(async ({
      log
    }, service) => {
      log('Wrapping...');
...
describe('useInject', () => {
  it('should set the right dependencies', () => {
    ...
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], [...fromDependencies]);
  });
});
describe('mergeInject', () => {
  it('should amend dependencies', () => {
    ...
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], fromDependencies);
    assert.notEqual(newInitializer[SPECIAL_PROPS.INJECT], toDependencies);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], [...toDependencies, ...fromDependencies]);
  });
});
describe('autoInject', () => {
  it('should allow to decorate an initializer with dependencies', () => {
...
describe('alsoInject', () => {
  ...
  it('should dedupe dependencies', () => {
    ...
    assert.notEqual(newInitializer, baseProvider);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], ['mysql', 'ENV', 'NODE_ENV', '?TEST', 'TEST2', 'db>mysql']);
  });
  it('should preserve single optional dependencies', () => {
    ...
    assert.notEqual(newInitializer, baseProvider);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], ['?TEST', '?TEST3', 'ENV', '?TEST2']);
  });
  it('should preserve mapped dependencies', () => {
    ...
    assert.notEqual(newInitializer, baseProvider);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], ['mysql', '?sftp', 'db>mysql', '?ftp>sftp']);
  });
  it('should solve dependencies alias name clash', () => {
    ...
    assert.notEqual(newInitializer, aProviderInitializer);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.INJECT], ['ENV', 'NODE_ENV', '?TEST', 'db>mysql', '?log>logly']);
  });
  it('should solve dependencies alias name clash', () => {
    const baseProvider = inject(['?TEST'], aProviderInitializer);
    const newInitializer = alsoInject(['ENV', '?NODE_ENV', '?TEST', 'db>mysql', '?log>logly'], alsoInject(['ENV', 'NODE_ENV', '?TEST', 'db>pg', '?log>logger'], baseProvider));
...
describe('extra', () => {
  ...
  it('should allow to decorate an initializer with additional extra infos', () => {
    ...
    assert.notEqual(newInitializer[SPECIAL_PROPS.EXTRA], baseExtraInformations);
    assert.deepEqual(newInitializer[SPECIAL_PROPS.EXTRA], { ...baseExtraInformations,
...
//# sourceMappingURL=util.test.mjs.map