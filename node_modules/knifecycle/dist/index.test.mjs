...
import { SPECIAL_PROPS, Knifecycle, initializer, inject, constant, service, provider, singleton } from './index';
...
describe('Knifecycle', () => {
  ...
  describe('register', () => {
    ...
    it('should fail with a bad service type', () => {
      assert.throws(() => {
        ...
        assert.deepEqual(err.params, ['test', 'not_allowed_type', ALLOWED_INITIALIZER_TYPES]);
        return true;
      });
    });
  ...
  describe('run', () => {
    ...
    it('should work with service dependencies', async () => {
      const wrappedSampleService = inject(['time'], async function sampleService({
        time
      }) {
  ...
  describe('$dispose', () => {
    ...
    it('should work with deeper dependencies', async () => {
      ...
      assert.deepEqual(Object.keys(dependencies), ['hash5', 'time', '$dispose', 'shutdownChecker']);
    ...
    it('should work with deeper multi used dependencies', async () => {
      ...
      assert.deepEqual(Object.keys(dependencies), ['hash1', 'hash2', '$dispose', 'shutdownChecker']);
      const finalPromise = shutdownCallPromise.then(() => {
        assert.deepEqual(shutdownStub.args, [[]]);
        shutdownResolve();
      });
    ...
    it('should delay service shutdown to their deeper dependencies', async () => {
      ...
      await dependencies.$dispose();
      assert.deepEqual(servicesShutdownCalls.args, [['hash2'], ['hash1'], ['hash']]);
    });
